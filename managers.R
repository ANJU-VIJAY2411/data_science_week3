...
missing_value <- aggr(managers_data , prop = "FALSE" , numbers = TRUE)
...
new_data_frame <- data.frame(managers_data[names(managers_data)[4]]
                             ,managers_data[names(managers_data)[5]])

new_data_frame


#extract all records where age > 35 or age < 24 and only list age and gender_col
